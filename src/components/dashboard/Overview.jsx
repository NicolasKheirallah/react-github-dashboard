...
const formatScopeLabel = (ownerScope, repoScope, timeRange) => {
  const rangeLabel =
    timeRange === '30d'
      ? 'last 30 days'
      : timeRange === '90d'
  ...
  if (ownerScope === 'all' && repoScope === 'all') {
    return `Showing ${rangeLabel} across all repositories`;
  }

  if (repoScope === 'all') {
    return `Showing ${rangeLabel} for ${ownerScope}`;
  }

  return `Showing ${rangeLabel} for ${repoScope}`;
};
...
const Overview = ({
  ...
  const starredRepos = scopedData?.starredRepos ?? allStarredRepos ?? [];
  ...
  const stats = {
    prs: {
      ...
      merged: pullRequests?.filter(pr => pr.state === 'merged')?.length || 0,
      closed: pullRequests?.filter(pr => pr.state === 'closed')?.length || 0,
    },
    issues: {
    ...
    repos: {
      ...
      public: repositories?.filter(repo => !repo.private && !repo.isPrivate)?.length || 0,
      private: repositories?.filter(repo => repo.private || repo.isPrivate)?.length || 0,
      forked: repositories?.filter(repo => repo.fork || repo.isFork)?.length || 0,
      sources: repositories?.filter(repo => !repo.fork && !repo.isFork)?.length || 0,
    },
    orgs: {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="md:ml-6 flex-grow">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between">
              ...
              <div className="mt-4 md:mt-0">
                <SafeExternalLink
                  href={userData?.login ? `https://github.com/${encodeURIComponent(userData.login)}` : null}
              ...
              <div className="flex flex-wrap gap-x-6 gap-y-2 mt-4 text-sm text-gray-600 dark:text-gray-400">
                ...
                {userData.twitter_username && (
                  <div className="flex items-center">
                    ...
                    <SafeExternalLink
                      href={`https://twitter.com/${encodeURIComponent(userData.twitter_username)}`}
                      className="text-blue-600 dark:text-blue-400 hover:underline"
                    >
                      @{userData.twitter_username}
                    </SafeExternalLink>
      ...
      <div className="flex flex-col gap-3 rounded-lg border border-blue-100 bg-blue-50/70 p-4 dark:border-blue-900/60 dark:bg-blue-950/30 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-base font-semibold text-slate-900 dark:text-white">
            Scope summary
          </h2>
          <p className="text-sm text-slate-600 dark:text-slate-300">
            {formatScopeLabel(ownerScope, repoScope, timeRange)}
          </p>
        </div>
        <div className="flex flex-wrap gap-2 text-sm text-slate-600 dark:text-slate-300">
          <span className="rounded-full bg-white px-3 py-1 font-medium shadow-sm dark:bg-slate-900/50">
            {stats.prs.open} open PRs
          </span>
          <span className="rounded-full bg-white px-3 py-1 font-medium shadow-sm dark:bg-slate-900/50">
            {stats.issues.open} open issues
          </span>
          <span className="rounded-full bg-white px-3 py-1 font-medium shadow-sm dark:bg-slate-900/50">
            {stats.repos.total} repos in scope
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
        <StatCard
          ...
          onClick={onJumpToTab ? () => onJumpToTab('pull-requests') : undefined}
          icon={
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        <StatCard
          ...
          onClick={onJumpToTab ? () => onJumpToTab('issues') : undefined}
          icon={
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        <StatCard
          ...
          onClick={onJumpToTab ? () => onJumpToTab('repositories') : undefined}
          icon={
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="space-y-2">
              {getTopLanguages(repositories, 5).map((lang) => (
...
const StatCard = ({ title, value, icon, details = [], onClick }) => {
  ...
  return (
    <Container
      ...
      onClick={onClick}
      className={`bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden text-left ${
        onClick
          ? 'transition hover:border-blue-400 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:hover:border-blue-500'
          : ''
      }`}
    >
      <div className="p-5">
